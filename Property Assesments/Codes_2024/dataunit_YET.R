...
property_dt <- read.csv("~/Desktop/BARI/PA-2023/base_data/fy2023-property-assessment-data.csv") #180627
...
names(SAM_dt)[27] <- "GIS_ID" # rename "PARCEL" column as "GIS_ID" 
...
#due to update in group_by function, I revised the below code
#A <- property_dt %>% group_by(.dots=names(property_dt)[-grep("unit_count", names(property_dt))]) %>% mutate(New_unit = sum(unit_count))
grouping_vars <- names(property_dt)[-grep("unit_count", names(property_dt))]
A <- property_dt %>%
  group_by(across(all_of(grouping_vars))) %>%
  mutate(New_unit = sum(unit_count))


# install.packages("gt") here to avoid warning.
...
A$unit_N_orig <- ifelse(property_dt$UNIT_NUM == "", NA, A$unit_N)
data_all <- A 
# this dataset has --- NA values of units- 177091 rows - we went from 23032 NAs to 5560
#data_all[76] <- NULL
#data_all[76] <- NULL
sum(is.na(data_all$unit_N)) 
#22908 in Aug 23 #5553 in Nov 23
#Skipped to remove columns in case we may need further.
remove_columns <- c("unit_count", "unit_N_org") #"New_unit"
...
# (by YET) calculate NAs by LU types
data.frame(num_NA = sapply(unique(data_all$LU), function(x) sum(is.na(data_all$unit_N[data_all$LU == x]))))
#num_NA
#R3           0
#R2           0
#CM           0
...
lm_md_gross_area =glm(GROSS_TAX ~ LIVING_AREA + GROSS_AREA, data=data_all[data_all$GROSS_TAX != 0,])
glm_pred_gross_area <- predict.glm(lm_md_gross_area,  newdata = data_all[data_all$GROSS_TAX == 0,], type = "response")

data_all[data_all$GROSS_TAX == 0,'GROSS_TAX'] <- as.integer(glm_pred_gross_area)
...
k$unit_N <- 1000
write.csv(k, '~/Desktop/BARI/PA-2023/outputs2/PA_outliers_24.csv', row.names= FALSE)
...
### WRITE CSV ###
write.csv(data_all_final, '~/Desktop/BARI/PA-2023/outputs2/PAD.Record.wUnit2024.csv', row.names=F)
# > 100267/175004
# [1] 0.5729412
