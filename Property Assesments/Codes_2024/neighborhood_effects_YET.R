...
# setwd("~/Google Drive/BARI Research Team Data Library/")

##################################################################
#BARI <- "D:/Google Drive/BARI Research Team Data Library/"
#PArecord_final_path = "Property Assessment Data/Outputs/PADCross.Record.082020.csv"
#prop.acs <- read_csv(paste0(BARI,PArecord_final_path))
#colnames(prop.acs)

#prop.acs <- read.csv("/Users/saina/Downloads/PADCross.Record.052021.csv")
PADcross <- read.csv("~/Desktop/BARI/PA-2023/outputs2/PADCross.Record.11012023.csv")
...
#census <- read.csv("/Users/saina/Downloads/ACS_1317_TRACT.csv")
#model <- left_join(model, census[,c("CT_ID_10", "PopDen")], by = c("CT_ID_10" = "CT_ID_10")) #
#sum(model$PopDen < 250, na.rm = T)

#model <- model[model$PopDen > 250,] #176636
...
nrow(model) #75233 # 71525 in 2023 #146879 in sep23 # 136462 in Nov 23


model <- model[model$AV_TOTAL < quantile(model$AV_TOTAL, 0.96, na.rm = T),] #72223 #68664 in 2023 # 141003 in Sep 23 #131003 in Nov 23
...
length(outlier_list) # dropping 10 CTS for not having enough observations #9 in 2023

###Subset listings data to remove outliers
nrow(model.mlm) #73464 in 2023 #63253 in Nov 23
model.mlm<-model.mlm[!(model.mlm$CT_ID_10 %in% outlier_list) ,]
nrow(model.mlm) #72108 #3742 in 2023 #73444 in Sep 23 #63228 in Nov 23
...
length(tract.unique$CT_ID_10) #155
tract.unique$CensusTract <- c(1:155) #Error in `$<-.data.frame`(`*tmp*`, CensusTract, value = 1:166) : 
...
tract.unique <- as.data.frame(tract.unique, row.names = FALSE)
rownames(tract.unique) <-NULL

tract.unique$random <- tract.unique$random$`(Intercept)`
tract.unique$random <- exp(tract.unique$random)

write.csv(tract.unique, "~/Desktop/BARI/PA-2023/outputs2/proptracts_effects_11012023.csv", row.names = F)
...
## Model for manipulation
model <- prop.acs %>%
  dplyr::select(PID,CT_ID_10,GIS_ID,
...
model.mlm <- subset(model, simplu != "CONDO_UNIT") %>%
  dplyr::select(propid, CT_ID_10, GIS_ID,simplu, 
...
model %>% 
  group_by(simplu) %>% 
  summarise(sumNA = sum(is.na(CT_ID_10)))
...
## Just Random Effects
randomodel <- lmer(AV_TOTAL ~ (1|CT_ID_10),
                   data = noout,
                   REML = F,
                   na.action = "na.omit")
summary(randomodel)
...
rando.coefs <- ranef(fullmodel)$CT_ID_10
head(rando.coefs)
colMeans(rando.coefs)
skim(rando.coefs)

## The Coefficients for the Full Model
coefs <- coef(fullmodel)$CT_ID_10
head(coefs)
...
test <- tract.unique$Intercept - ranef(fullmodel)$CT_ID_10

tract.unique$CT_ID_10 <- as.numeric(tract.unique$CT_ID_10)
# proptracts_effects <- left_join(proptracts, tract.unique, by= "CT_ID_10")
proptracts_effects <- merge(proptracts, tract.unique, by= "CT_ID_10", all=F)
