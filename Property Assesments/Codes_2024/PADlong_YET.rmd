...
### Cleaning Functions ###
source("/Users/saina/Downloads/Cleaning_functions_20190326.R")
source("/Users/saina/Downloads/Geocoding_functions_20190326.R")
source("/Users/saina/Downloads/Helper_functions_20190326.R")


### INPUT PATHS ###
# OUTPUT PATHS
pal_path = "PADLong.Record.072022.csv"
pal_ct_path = "PADLong.CT.072022.csv"
pal_ct_shp_path = "Tract Shp Long"
pal_ct_shp_name = "PADLong.CT.07272022"

pal_bg_path = "PADLong.CBG.072022.csv"
pal_bg_shp_path = "BG Shp Long"
pal_bg_shp_name = "PADLong.CBG.27072022"

# READ IN FILES

palold = read.csv("/Users/saina/Downloads/PADLong.Record.082020.csv")
palold19 = read.csv("/Users/saina/Downloads/PADLong.Record.2019.csv")
palold18 = read.csv("/Users/saina/Downloads/PADLong.Record.2018.csv")
# palold = read.csv("/Users/saina/Downloads/PADLong.Record.052021 (1).csv")
PA <- read.csv("/Users/Shared/Files From d.localized/School/Semester 6/BARI PAD update/PAD/PADCross.Record.DRFT1.06202022.csv")
bgsShp <- st_read("/Users/saina/Downloads/Block Groups 2010 BARI/Census Block Groups.shp")
ctsShp <- st_read("/Users/saina/Downloads/Tracts_Boston_2010_BARI (1)/Tracts_Boston BARI.shp")
IDconnector <- read.csv("/Users/saina/Downloads/IDConnector2020.csv") #390321
landParcels <- read.csv("/Users/saina/Downloads/Parcel_final_2021_9302021_postsanitychecks (2).csv")
...
# just for this once
parcep_1819 <- setdiff(palold18$Land_Parcel_ID, palold19$Land_Parcel_ID)
palold18 <- palold18[palold18$Land_Parcel_ID %in% parcep_1819,] #220
# << this has to be added to 2019

parcep_1920 <- setdiff(palold19$Land_Parcel_ID, palold$Land_Parcel_ID)
palold19 <- palold19[palold19$Land_Parcel_ID %in% parcep_1920,] #678
# << this has to be added to 2020

# let's combine the two
palold18$Blk_ID_10 <- as.character(palold18$Blk_ID_10)
palold19$Blk_ID_10 <- as.character(palold19$Blk_ID_10)
palold18$BG_ID_10 <- as.character(palold18$BG_ID_10)
palold19$BG_ID_10 <- as.character(palold19$BG_ID_10)
palold18$CT_ID_10 <- as.character(palold18$CT_ID_10)
palold19$CT_ID_10 <- as.character(palold19$CT_ID_10)

leftovers <- bind_rows(palold18, palold19)
leftovers <- distinct(leftovers) #891
...
#176988
parcep_20 <- setdiff(leftovers$Land_Parcel_ID, palold$Land_Parcel_ID) #177
leftovers <- leftovers[leftovers$Land_Parcel_ID %in% parcep_20,] #772
palold <- bind_rows(palold, leftovers) #177760
palold <- palold[,c(-6,-7)]
...
pal$Land_Parcel_ID[is.na(pal$Land_Parcel_ID) & !is.na(pal$PID)] <- palold19$Land_Parcel_ID[!is.na(palold19$Land_Parcel_ID)][match(pal$PID[is.na(pal$Land_Parcel_ID) & !is.na(pal$PID)], palold19$PID)] 

pal$GIS_ID[is.na(pal$GIS_ID) & !is.na(pal$PID)] <- palold19$GIS_ID[!is.na(palold19$GIS_ID)][match(pal$PID[is.na(pal$GIS_ID) & !is.na(pal$PID)], palold19$PID)] 

pal$CT_ID_10[is.na(pal$CT_ID_10) & !is.na(pal$PID)] <- palold19$CT_ID_10[!is.na(palold19$CT_ID_10)][match(pal$PID[is.na(pal$CT_ID_10) & !is.na(pal$PID)], palold19$PID)] 

pal$BG_ID_10[is.na(pal$BG_ID_10) & !is.na(pal$PID)] <- palold19$BG_ID_10[!is.na(palold19$BG_ID_10)][match(pal$PID[is.na(pal$BG_ID_10) & !is.na(pal$PID)], palold19$PID)] 

pal$Blk_ID_10[is.na(pal$Blk_ID_10) & !is.na(pal$PID)] <- palold19$Blk_ID_10[!is.na(palold19$Blk_ID_10)][match(pal$PID[is.na(pal$Blk_ID_10) & !is.na(pal$PID)], palold19$PID)] 

pal$CM_ID[is.na(pal$CM_ID) & !is.na(pal$PID)] <- palold19$CM_ID[!is.na(palold19$CM_ID)][match(pal$PID[is.na(pal$CM_ID) & !is.na(pal$PID)], palold19$PID)] 

pal$ST_NUM[is.na(pal$ST_NUM) & !is.na(pal$PID)] <- palold19$ST_NUM[!is.na(palold19$ST_NUM)][match(pal$PID[is.na(pal$ST_NUM) & !is.na(pal$PID)], palold19$PID)] 

pal$ST_NAME_SUF[is.na(pal$ST_NAME_SUF) & !is.na(pal$PID)] <- palold19$ST_NAME_SUF[!is.na(palold19$ST_NAME_SUF)][match(pal$PID[is.na(pal$ST_NAME_SUF) & !is.na(pal$PID)], palold19$PID)] 

pal$ZIPCODE[is.na(pal$ZIPCODE) & !is.na(pal$PID)] <- palold19$ZIPCODE[!is.na(palold19$ZIPCODE)][match(pal$PID[is.na(pal$ZIPCODE) & !is.na(pal$PID)], palold19$PID)] 

pal$TLID[is.na(pal$TLID) & !is.na(pal$PID)] <- palold19$TLID[!is.na(palold19$TLID)][match(pal$PID[is.na(pal$TLID) & !is.na(pal$PID)], palold19$PID)] 

pal$X[is.na(pal$X) & !is.na(pal$PID)] <- palold19$X[!is.na(palold19$X)][match(pal$PID[is.na(pal$X) & !is.na(pal$PID)], palold19$PID)] 

pal$Y[is.na(pal$Y) & !is.na(pal$PID)] <- palold19$Y[!is.na(palold19$Y)][match(pal$PID[is.na(pal$Y) & !is.na(pal$PID)], palold19$PID)] 
...
palold19$CT_ID_10 <- trimws(palold19$CT_ID_10)
palold19$CT_ID_10[which(palold19$CT_ID_10 == "NA")] <- NA
palold19$BG_ID_10 <- trimws(palold19$BG_ID_10)
palold19$BG_ID_10[which(palold19$BG_ID_10 == "NA")] <- NA
palold19$Blk_ID_10 <- trimws(palold19$Blk_ID_10)
palold19$Blk_ID_10[which(palold19$Blk_ID_10 == "NA")] <- NA
palold19$TLID <- trimws(palold19$TLID)
palold19$TLID[which(palold19$TLID == "NA")] <- NA
palold19$Land_Parcel_ID <- trimws(palold19$Land_Parcel_ID)
palold19$Land_Parcel_ID[which(palold19$Land_Parcel_ID == "NA")] <- NA
palold19$GIS_ID <- trimws(palold19$GIS_ID)
palold19$GIS_ID[which(palold19$GIS_ID == "NA")] <- NA
...
view(palold19[palold19$PID %in% A,])
...
view(palold18[palold18$parcel_num %in% A,])

names(palold18)[1] <- "PID"

pal$X[is.na(pal$X) & !is.na(pal$PID)] <- palold18$X[!is.na(palold18$X)][match(pal$PID[is.na(pal$X) & !is.na(pal$PID)], palold18$PID[!is.na(palold18$X)])]

pal$Y[is.na(pal$Y) & !is.na(pal$PID)] <- palold18$Y[!is.na(palold18$Y)][match(pal$PID[is.na(pal$Y) & !is.na(pal$PID)], palold18$PID[!is.na(palold18$Y)])]

pal$CT_ID_10[is.na(pal$CT_ID_10) & !is.na(pal$PID)] <- palold18$CT_ID_10[!is.na(palold18$CT_ID_10)][match(pal$PID[is.na(pal$CT_ID_10) & !is.na(pal$PID)], palold18$PID[!is.na(palold18$CT_ID_10)])]

pal$BG_ID_10[is.na(pal$BG_ID_10) & !is.na(pal$PID)] <- palold18$BG_ID_10[!is.na(palold18$BG_ID_10)][match(pal$PID[is.na(pal$BG_ID_10) & !is.na(pal$PID)], palold18$PID[!is.na(palold18$BG_ID_10)])]

pal$Blk_ID_10[is.na(pal$Blk_ID_10) & !is.na(pal$PID)] <- palold18$Blk_ID_10[!is.na(palold18$Blk_ID_10)][match(pal$PID[is.na(pal$Blk_ID_10) & !is.na(pal$PID)], palold18$PID[!is.na(palold18$Blk_ID_10)])]

pal$TLID[is.na(pal$TLID) & !is.na(pal$PID)] <- palold18$TLID[!is.na(palold18$TLID)][match(pal$PID[is.na(pal$TLID) & !is.na(pal$PID)], palold18$PID[!is.na(palold18$TLID)])]

pal$GIS_ID[is.na(pal$GIS_ID) & !is.na(pal$PID)] <- palold18$GIS_ID[!is.na(palold18$GIS_ID)][match(pal$PID[is.na(pal$GIS_ID) & !is.na(pal$PID)], palold18$PID[!is.na(palold18$GIS_ID)])]

pal$Land_Parcel_ID[is.na(pal$Land_Parcel_ID) & !is.na(pal$PID)] <- palold18$Land_Parcel_ID[!is.na(palold18$Land_Parcel_ID)][match(pal$PID[is.na(pal$Land_Parcel_ID) & !is.na(pal$PID)], palold18$PID[!is.na(palold18$Land_Parcel_ID)])]
...
long$ST_NAME_SUF[is.na(long$ST_NAME_SUF)] <- palold18$ST_NAME_SUF[match(long$PID[is.na(long$ST_NAME_SUF)], palold18$PID)]
names(long)
