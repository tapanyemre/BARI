...
PADcross <- read.csv("~/Desktop/BARI/PA-2023/outputs_new/PADCross.Record.DRFT1.08162023.csv")
...
model <- model[complete.cases(model),] #75233
...
names(model) #
nrow(model) #75233 # 71525 in 2023
model <- model[model$AV_TOTAL < quantile(model$AV_TOTAL, 0.96, na.rm = T),] #72223 #68664 in 2023
...
length(outlier_list) # dropping 10 CTS for not having enough observations

###Subset listings data to remove outliers
nrow(model.mlm)
model.mlm<-model.mlm[!(model.mlm$CT_ID_10 %in% outlier_list) ,]
nrow(model.mlm) #72108 #3742 in 2023
...
length(tract.unique$CT_ID_10)
tract.unique$CensusTract <- c(1:166)
...
write.csv(tract.unique, "/Users/Shared/Files From d.localized/School/Semester 6/BARI PAD update/PAD/proptracts_effects_07032022.csv", row.names = F)
