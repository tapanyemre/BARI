...
BARI <- "~/Desktop/BARI/PA-2023/outputs2/"
...
PADunit <- read.csv("PA-2023/outputs2/PAD.Record.wUnit2024.csv")
#bgs_10 <- st_read("~/Desktop/BARI/PA-2023/base_data/Blocks_Boston_2010_BARI/Blocks_Boston BARI.shp")
#cts_10 <- st_read("~/Desktop/BARI/PA-2023/base_data/Tracts_Boston_2010_BARI/Tracts_Boston BARI.shp") %>% dplyr::select(CT_ID_10)
cts_10 <- tigris::tracts(state = "MA", county = "Suffolk", year = 2019) %>% dplyr::select(CT_ID_10 = GEOID)
cts_20 <- tigris::tracts(state = "MA", county = "Suffolk", year = 2022) %>% dplyr::select(CT_ID_20 = GEOID)
bgs_10 <- tigris::block_groups(state = "MA",county = "Suffolk",year = 2019) %>%  dplyr::select(BG_ID_10 = GEOID)
bgs_20 <- tigris::block_groups(state = "MA",county = "Suffolk",year = 2022) %>%  dplyr::select(BG_ID_20 = GEOID)
properties <- read.csv("~/Desktop/BARI/GI-2023/drive/properties_geo_10032023_YET_postsanitychecks.csv")
#???????????j <- read.csv("/Users/saina/Downloads/proptracts_effects_2021_SS (1).csv")

# OUTPUT PATHS
PArecord_final_path = "PADCross.Record.11012023.csv"
pa_ct_path = "PADCross.CT.11012023.csv"
pa_ctshp_path = "Tract Shp"
pa_ctshp_name = "PADCross.CT.11012023"

pa_bg_path = "PADCross.CBG.11012023.csv"
pa_bgshp_path = "BG Shp"
pa_bgshp_name = "PADCross.CBG.11012023"
...
PAclean <- distinct(PAclean) # 177091 #107591 in 2023 #180626 in Sep 23
...
PA$Blk_ID_20 <- format(PA$Blk_ID_20, scientific = F)
PA$BG_ID_20 <- format(PA$BG_ID_20, scientific = F)
PA$CT_ID_20 <- format(PA$CT_ID_20, scientific = F)
...
PA$Blk_ID_20 <- trimws(PA$Blk_ID_20)
PA$CT_ID_20 <- trimws(PA$CT_ID_20)
PA$BG_ID_20 <- trimws(PA$BG_ID_20)
...
PA$CT_ID_20[which(PA$CT_ID_20 == "NA")] <- NA
PA$Blk_ID_20[which(PA$Blk_ID_20 == "NA")] <- NA
PA$BG_ID_20[which(PA$BG_ID_20 == "NA")] <- NA
...
varnames = c("parcel_num","CM_ID","GIS_ID","ST_NUM" ,"ST_NAME_SUFF","UNIT_NUM",
             ...
             "BLDG_AGE","X","Y","CITY","LU_DESC","CD_FLOOR","INT_WALL","KITCHENS","U_TOTAL_RMS","BDRM_COND","HEAT_FUEL" ,
...
# names(TLID)[5] <- "GIS_ID"
TLID <- read.csv("~/Desktop/BARI/GI-2023/drive/Parcel_final_10032023_YET_postsanitychecks.csv") #98146 #98150 in 2023 #97894 in Sep 23
...
PADcross <- rename(PADcross, BTHRM_STYLE2= R_BTH_STYLE2)
PADcross <- rename(PADcross, BTHRM_STYLE3= R_BTH_STYLE3)
...
PADcross$X.1 <- 1:nrow(PADcross)
PADcross <- rename(PADcross, KITCHEN = KITCHENS)
PADcross <- rename(PADcross, ZIPCODE = ZIP_CODE)
...
address_splits <- t(sapply(PADcross$`OWNER.MAIL.ADDRESS`, split_address))
...
PADcross <- cbind(PADcross, address_df)
#PADcross$`OWNER.MAIL.ADDRESS` <- NULL
PADcross <- rename(PADcross, MAIL_ADDRESSEE = OWNER.MAIL.ADDRESS)
#Extract C/O pattern
extract_CO_and_address <- function(str) {
  ...
  return(list(CO_Info = CO_info, Address = str_trim(address)))
}

extracted_list <- lapply(PADcross$`MAIL_ADDRESS`, extract_CO_and_address)
extracted_data <- do.call(rbind.data.frame, extracted_list)
...
PADcross$MAIL_ADDRESS <- extracted_data$Address
PADcross$MAIL_ADDRESSEE <- extracted_data$CO_Info

oldpad <- read.csv("~/Desktop/BARI/PA-2023/base_data/2022/PADCross.Record.07132022.csv")
setdiff(names(oldpad), names(PADcross))
setdiff(names(PADcross), names(oldpad))
# Exclude the undesired column
cols <- names(oldpad)[names(oldpad) != "PlUMBING"]
# Append additional column names
final_cols <- c(cols, "Blk_ID_20", "BG_ID_20", "CT_ID_20")
PADcross <- PADcross[,final_cols]
tmp <- PADcross$X
PADcross$X <- PADcross$Y
PADcross$Y <- tmp
rm(tmp)

#write.csv(PA,paste0(BARI,PArecord_final_path),row.names=F)
write.csv(PADcross,"~/Desktop/BARI/PA-2023/outputs2/PADCross.Record.11012023.csv",row.names=F)
...
#PA_PA <- read.csv("~/Desktop/BARI/PA-2023/outputs/PADCross.Record.DRFT1.09212023.csv")
#PA <- PA_PA
#PA = PA
PA <- read.csv("~/Desktop/BARI/dataverse_PA/PADCross.Record.2022.csv")
#HEAT, COOL, EE missing: all but R1-3
...
tract.unique <- read.csv("~/Desktop/BARI/PA-2023/outputs2/others/proptracts_effects_11012023.csv")
...
st_write(PA.agg.CT.shp,  "PADCross.CT.10", driver="ESRI Shapefile", 
         overwrite_layer = T)
...
tract.unique <- read.csv("~/Desktop/BARI/dataverse_PA/tract effects/proptracts_effects_2020.csv")
...
#PA.agg.CT_na <- PA.agg.CT


# # Apply the function to your data frame
# PA.agg.CT_na <- replaceNaNwithNA(PA.agg.CT_na)
# PA.agg.CT_final <- PA.agg.CT_na
# PA.agg.CT_final$DEC_BUILT_REMOD.res <- sapply(PA.agg.CT_na$DEC_BUILT_REMOD.res, `[`, 1)
# PA.agg.CT_final$DEC_BUILT_REMOD.nonres <- sapply(PA.agg.CT_na$DEC_BUILT_REMOD.nonres, `[`, 1)
# list <- c("25025000202", "25025070801", "25025981600")

#PA.agg.CT_final$DEC_BUILT_REMOD.nonres[which(PA.agg.CT_final$CT_ID_20 %in% list)] <- 2010
...
PA.agg.CBG.rename = rename(PA.agg.CBG, EESR = EE_SCORE.res, 
