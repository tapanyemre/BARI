...
data <- read.csv("~/Desktop/Academic/BARI/911/Final/911 Calls From Jan2023-Dec2023_with_LatLon.csv")
...
data <- data911_1[,c("eid", "date", "year", "TYPE", "SocDis", "Guns", "PrivateConflict", "Violence", 
                "Larceny" ,        "Burglary", "Vandalism",       "Robbery" ,
                "Land_Parcel_ID", "TLID", "Blk_ID_10", "BG_ID_10", "CT_ID_10", #"sj_cat",
                "Blk_ID_20", "BG_ID_20", "CT_ID_20", "pre_sys_change")]
data <- data[,-c(22:23)]
...
min(data$Blk_ID_20, na.rm = T)
min(data$BG_ID_20, na.rm = T)
min(data$CT_ID_20, na.rm = T)
...
time.periods <- sort(unique(data$year))
...
pop.ct <- read.csv("~/Desktop/Academic/BARI/911/Data/NEW/DEC_CENSUS_2020_TRACT.csv", header=T)
pop.cbg <- read.csv("~/Desktop/Academic/BARI/911/Data/NEW/DEC_CENSUS_2020_BLKGRP.csv", header =T)
pop.blk <-  read.csv("~/Desktop/Academic/BARI/911/Data/NEW/DEC_CENSUS_2020_BLK.csv", header =T)
...
# pop.ct <- list()
# pop.ct[[1]] <- read.csv("911/Data/NEW/DEC_CENSUS_2020_TRACT.csv", header=T)
# # pop.ct[[2]] <- read.csv("ACS_1418_TRACT.csv", header=T)
# 
# pop.cbg <- list()
# pop.cbg[[1]] <- read.csv("911/Data/NEW/DEC_CENSUS_2020_BLKGRP.csv", header =T)
...
# pop.blk <- list()
# pop.blk[[1]] <-  read.csv("911/Data/NEW/DEC_CENSUS_2020_BLK.csv", header =T)

# blk.pop <- read.csv("~/Desktop/Academic/BARI/911/Data/Blocks/Blocks_Boston_2020_BARI.csv", header = T)
# blk.pop$Blk_ID_20 <- blk.pop$GEOID20
...
data$CAT <- ifelse(data$Larceny==1, "Larceny", data$CAT)
data$CAT <- ifelse(data$Burglary==1, "Burglary", data$CAT)
data$CAT <- ifelse(data$Vandalism==1, "Vandalism", data$CAT)
data$CAT <- ifelse(data$Robbery==1, "Robbery", data$CAT)
...
for (j in 1:length(time)) {
  for (i in 1:length(geo))  {
    ...
    temp <- subset(temp, temp[,colnames(temp) == geo[i]] %in% unique(cat.freq.3$geo.var))
    temp <- temp[, colnames(temp) %in% c(geo[i], "TotalPop")]
    #datalist[[1]] <- merge(temp, cat.freq.3[cat.freq.3$variable %in% vars[grep(time.periods[1], vars)],], by.x = geo[i], by.y = "geo.var", all.y = TRUE)
