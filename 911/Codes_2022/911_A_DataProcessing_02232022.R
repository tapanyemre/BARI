...
common_event <- read.csv("2021_Common_Event.csv")
agency_event <- read.csv("2021_AGENCY_EVENT.csv")
...
par <- st_read("Parcels2018_11132019.shp")
parcels_buffer50m <- st_read("parcels_boundary_buffer50m.shp")
road <- st_read("roads_fin_clust_2019_11142019.shp")
# block
cb <- st_read("BlocksBostonBARI.shp")

# name for the formatted NEW data dump alone, fill in "***"
date <- as.character("Dec2021")  # today's date, for saving
save_name <- paste("911 Calls From Jan2021-_", date, ".csv", sep="")
...
file_start_date <- "2021-01-01 00:00:00"
save_name_updated <- paste("911 Calls_Full_2021_", date, ".csv", sep="") # example file name
...
typ <- read.csv("911_Types_2014_to_2020.csv", header=T, stringsAsFactors = F)
...
typloc <- "C:\\Users\\daveh\\Documents\\Research\\BARI\\bari_911_prep"
...
colnames(par)[2] <- "Land_Parcel_ID"
par <- merge(par, parcels.2018[,c("Land_Parcel_ID", "TLID", "Blk_ID_10", "BG_ID_10", "CT_ID_10", "NSA_Name", "BRA_PD")], 
             by = "Land_Parcel_ID", all.x = T)
...
#previous <- read.csv(existing, header = T)
...
length(setdiff(ae$tTYPE, typ$TYPE))
length(setdiff(typ$tTYPE, ae$TYPE))
x1 <- ae %>% group_by(TYPE) %>% dplyr::summarise(n()) # for 2019-2020 there are 322 obs ##2021 - 297 obs
x2 <- typ %>% group_by(TYPE) %>% dplyr::summarise(n()) # for 2014-2020 half: there are 341 obs ##2021 - 347 obs
...
length(unique(ae$TYPE[!ae$TYPE %in% typ$TYPE])) #6 in 2021 January update same in 2021 full year update
...
names_types <- c("DISTRBDRUNKS", "DISTRBPANHAN", "IVPERLEWD", "VANDIP")
...
names_types <- c("LANTENDEFAULT", "VANDRPT", "VIORDRDEFAULT", "BEIP")
...
typs.new$Frequency_2020 <- NA
typs.new$Frequency_2021 <- NA
...
for (i in 1:length(types)){
  ...
  typs.new$Frequency_2020[i] <- ifelse(types[i] %in% d911$TYPE,
                                      length(unique(d911$eid[d911$TYPE == types[i] & d911$year == 2020])),
                                                                            0)
  typs.new$Frequency_2021[i] <- ifelse(types[i] %in% d911$TYPE,
                                       length(unique(d911$eid[d911$TYPE == types[i] & d911$year == 2021])),
                                       0)
}
...
typs.new$Used <- ifelse(typs.new$SocDis == 1 | typs.new$PrivateConflict == 1 | typs.new$Violence == 1 | typs.new$Guns == 1, 1, 0)
...
typs.new1_2020 <- typs.new1[typs.new1$last.yr == 2020,]
typs.new1_2021 <- typs.new1[typs.new1$last.yr == 2021,]
...
typs_join$Frequency_2020 <- typs.new$Frequency_2020[match(typs_join$TYPE, typs.new1$TYPE)]
typs_join$Frequency_2021 <- typs.new$Frequency_2021[match(typs_join$TYPE, typs.new1$TYPE)]

typs_join$last.yr <- ifelse(typs_join$TYPE %in% typs.new1_2019$TYPE, 2019, typs_join$last.yr)
typs_join$last.yr <- ifelse(typs_join$TYPE %in% typs.new1_2020$TYPE, 2020, typs_join$last.yr)
typs_join$last.yr <- ifelse(typs_join$TYPE %in% typs.new1_2021$TYPE, 2021, typs_join$last.yr)
...
typs_to_add <- typs_to_add %>%
  ...
  dplyr::select(TYPE, tycod, typ_eng, sub_tycod, sub_eng, SocDis, PrivateConflict, Violence, Guns, 
         Frequency_2015, Frequency_2016, Frequency_2017, Frequency_2018, Frequency_2019, Frequency_2020, Frequency_2021, yr.intro, last.yr)
...
setwd(typloc)
write.csv(typs_join, paste("911_Types_2014_to", date, ".csv", sep = "_"), row.names = F)
...
ae <- merge(ae, 
            ...
            typs.new[,c("TYPE", "SocDis", "PrivateConflict", "Violence", "Guns")], # NOTE AR: typ does not include the ecometrics; change to typs.new
            by="TYPE", all.x = T)
...
parcels_buffer50m <- st_transform(parcels_buffer50m, 4326)

## there were multiple trials ##
## we keep the following ## it takes ~ 40 min ~30 SECONDS 01202021 ??
start_time <- Sys.time()
ce.togeo.proj1 <- ce.togeo.proj %>% 
  filter(st_contains(parcels_buffer50m, ., sparse = FALSE))
end_time <- Sys.time()
end_time - start_time
...
geo.data <- st_intersection(ce.togeo.proj1, st_make_valid(par[,c("Land_Parcel_ID","TLID" ,"Blk_ID_10", "BG_ID_10", "CT_ID_10", "NSA_Name", "BRA_PD")])) # problems with the polygons
end_time <- Sys.time()
end_time - start_time
...
start_time <- Sys.time()
ce.geo <- left_join(ce.togeo.proj1, geo.data, by="eid", all.x=T)
end_time <- Sys.time()
end_time - start_time
...
write.csv(data911, save_name, row.names = F)

### If we want to save only the ones inside the boundary of Boston and have minimum one geo characteristic ###
data911_1 <- data911[!is.na(data911$CT_ID_10) | !is.na(data911$BG_ID_10) | !is.na(data911$Blk_ID_10) |
               !is.na(data911$Land_Parcel_ID) | !is.na(data911$TLID),]
write.csv(data911_1, "911 Calls From Jan2021-Dec2021_with_LatLon.csv", row.names = F)
