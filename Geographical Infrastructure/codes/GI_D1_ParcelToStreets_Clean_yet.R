...
parcels <- read.csv("Scripts/GI/Outputs/Parcel_final_postsanitychecked_2024.csv")
parcel_shp <- st_read("~/Desktop/Academic/BARI/Scripts/GI/Outputs/BARIParcels2024/BARIParcels2024.shp")
...
roads <- read.csv("Scripts/GI/Inputs/Roads.2023.csv")
roads_shp <- st_read("~/Desktop/Academic/BARI/Scripts/GI/Inputs/Roads.2023/Roads.2023.shp")
...
prop <- read.csv("Scripts/GI/Inputs/Properties.2023.csv")

streets_prefinal <- roads

## counts of units, properties, and parcels ##
length(unique(parcels$TLID)) #13836 (parcels from script 03) #13890 new #13885 in 2019 #13827 in 2020 #13833 in 2020 with changes 
#2021: 13817 #13881 in 2023 Dec, 13866 in late2023 #13874 in 2024
...
# 13210 in 2021 #13273 in 2023 Dec 13259 in late2023 #13874 in 2024
...
streets_prefinal <- streets_prefinal[,-c(21:23)] #kept the parcel_N
...
#summary()
streets_prefinal$parcel_N_final <-ifelse(is.na(streets_prefinal$parcel_N.y),
                                         streets_prefinal$parcel_N.x,
                                         streets_prefinal$parcel_N.y)
streets_prefinal <- dplyr::select(streets_prefinal, -c("parcel_N.x", "parcel_N.y")) 
streets_prefinal <- dplyr::rename(streets_prefinal, parcel_N = parcel_N_final)

summary(streets_prefinal) # 11056 streets table(is.na(streets_prefinal$unit_N)) because 11056 street segments do not have parcels #11002 new #11007 in 2019 #11065 in 2020 
#11059 in 2020 #11075 in 2021 #11619 in Dec 2023 #11633 # in 2024 11018
table(is.na(streets_prefinal$unit_N))
# ####replace clusters#### 
...
streets_tracts_10 <- aggregate(CT_ID_10~TLID, data=parcels, FUN=Mode)
streets_bgs_10 <- aggregate(BG_ID_10~TLID, data=parcels, FUN=Mode)
...
streets_census_10 <-merge(streets_tracts_10,streets_bgs_10,by='TLID',all.x=TRUE) #13832 streets_census #13886 new #13881 in 2019 #13823 in 2020 #13828 in fixed one # 13813 in 2021 #13863 in Sep 2023 #13871 in 2024
streets_census_20 <-merge(streets_tracts_20,streets_bgs_20,by='TLID',all.x=TRUE) #13832 streets_census #13886 new #13881 in 2019 #13823 in 2020 #13828 in fixed one # 13813 in 2021 #13863 in Sep 2023 #13872 in 2024

list_of_dfs <- list(streets_tracts_10, streets_bgs_10, streets_tracts_20, streets_bgs_20)
streets_census <- Reduce(function(x, y) merge(x, y, by = "TLID", all = TRUE), list_of_dfs) #13864 in Dec 23 #13872 in 2024
# streets_census_all <- streets_census_all[-12302,]
# streets_census_10$CT_ID_10 <- map_dbl(streets_census_10$CT_ID_10, ~ .x[1])
# streets_census_10$BG_ID_10 <- map_dbl(streets_census_10$BG_ID_10, ~ .x[1])
# streets_census_20$CT_ID_20 <- map_dbl(streets_census_20$CT_ID_20, ~ .x[1])
# streets_census_20$BG_ID_20 <- map_dbl(streets_census_20$BG_ID_20, ~ .x[1])
...
sum(is.na(streets_census$BG_ID_10)) #0 in Sep 2023 #1 in 2024
# there is one NA in BG in 2018 / no NA in 2019 data # 1 in 2021
View(streets_census[which(is.na(streets_census$BG_ID_10)),]) #no NA in Sep 2023 #1 in 2024 (12305)
...
View(prop[which(prop$BG_ID_20 == 250250406001 & prop$TLID == 85736902),]) #two properties have these values #missing in 2024
...
length(setdiff(streets_census$BG_ID_10, streets_prefinal$BG_ID_10)) #1 in 2023 #0 in 2024
setdiff(streets_census$BG_ID_10, streets_prefinal$BG_ID_10) #250259901010 in 2023 #0 in 2024
setdiff(streets_census$CT_ID_10, streets_prefinal$CT_ID_10) #25025990101 in 2023 #0 in 2024

streets_census_all <- streets_census
...
streets_census <- merge(streets_prefinal,streets_census_10,by='TLID',all.x=TRUE)
streets_census <- merge(streets_census,streets_census_20,by='TLID',all.x=TRUE)
...
streets_census$BG_ID_20<-ifelse(is.na(streets_census$BG_ID_20.y),streets_census$BG_ID_20.x,as.numeric(as.character(streets_census$BG_ID_20.y)))
streets_census$CT_ID_20<-ifelse(is.na(streets_census$CT_ID_20.y),streets_census$CT_ID_20.x,as.numeric(as.character(streets_census$CT_ID_20.y)))

streets_census <- dplyr::select(streets_census, -c("BG_ID_10.x", "CT_ID_10.x", "CT_ID_10.y",  "BG_ID_10.y",
                                                   "BG_ID_20.x", "CT_ID_20.x", "CT_ID_20.y",  "BG_ID_20.y")) 
summary(streets_census)
# check the NAs
sum(is.na(streets_census$BG_ID_10))
...
# 155 #24 in 2020  # 24 in 2021 #24 in 2023 #24 in 2024
...
#streets_prefinal<-merge(streets_prefinal,streets_census[, c("TLID", "BG_ID_10", "CT_ID_10", "BG_ID_20", "CT_ID_20")],by='TLID',all.x=TRUE)
...
# 159 #0 #24 in 2024
sum(is.na(streets_prefinal$BG_ID_20))
# 155 #0 #0 in 2024
sum(is.na(streets_prefinal$CT_ID_10))
# 159 #0 #24 in 2024
sum(is.na(streets_prefinal$CT_ID_20))
# 155 #0 #0 in 2024

length(setdiff(streets_census$BG_ID_10, streets_census_all$BG_ID_10)) #2
length(setdiff(streets_census$CT_ID_10, streets_census_all$CT_ID_10)) #0
length(setdiff(streets_census$BG_ID_20, streets_census_all$BG_ID_20)) #16
length(setdiff(streets_census$CT_ID_20, streets_census_all$CT_ID_20)) #12
...
sum(is.na(streets_prefinal$BG_ID_10)) #24 in 2024
sum(na.omit(streets_prefinal)$BG_ID_10 == 0) # 0 in 2023
sum(streets_prefinal$BG_ID_10 == 0) #24 in 2023 #NA in 2024
# 159 #24
sum(is.na(streets_prefinal$CT_ID_10)) #24 in 2024
sum(streets_prefinal$CT_ID_10 == 0) #NA in 2024
# 159 #24 
sum(is.na(streets_prefinal$CT_ID_20)) #0 in 2024
sum(is.na(streets_prefinal$BG_ID_20)) #0 in 2024

#streets_prefinal[streets_prefinal$CT_ID_10 == 0, "CT_ID_10"] <- NA
#streets_prefinal[streets_prefinal$BG_ID_10 == 0, "BG_ID_10"] <- NA
#sum(streets_prefinal$unit_N == 0)
...
# names(parcel_shp)[2] <- "Land_Parcel_ID"
# # parcel_shp$Land_Parcel_ID <- parcel_shp$Ln_P_ID
# landparcels <- left_join(parcel_shp, parcels, by = "Land_Parcel_ID")
# # #coordinates(parcels) <- ~X + Y
# # #proj4string(parcels) = CRS("+init=epsg:4326")
# # parcels <- st_as_sf(landparcels_sub, coords = c("X", "Y"), crs = 26986)
# landparcels <- st_transform(landparcels, 26986)
# 
# streets_prefinal <- roads_23
# # merge streets_prefinal with roads for having geometry
streets_prefinal_shp <- left_join(streets_prefinal, roads_shp[c( "TLID", "geometry")], by="TLID")
streets_prefinal_shp <- st_as_sf(streets_prefinal_shp, crs = 26986)
streets_prefinal_shp <- st_transform(streets_prefinal_shp, 26986)
# 
...
# roads_shp_noBG <- streets_prefinal_shp[is.na(streets_prefinal_shp$BG_ID_10),] #24
# roads_shp_noBG <- st_as_sf(roads_shp_noBG)
# roads_shp_noBG <- st_transform(roads_shp_noBG, 26986)
# plot(roads_shp_noBG[,1])
...
# library(nngeo)
# # Remove rows with missing coordinates and convert to sf
# parcels_sf <- parcels %>%
#   filter(!is.na(X) & !is.na(Y)) %>%
#   st_as_sf(coords = c("X", "Y"), crs = 26986)
# dist <- st_join(roads_shp_noBG, parcels_sf[,c("BG_ID_10", "CT_ID_10", "geometry")], st_nn, k = 1, maxdist = 50)
# # 
# 
# dist_filled <- dist %>%
#   mutate(
#     BG_ID_10 = coalesce(BG_ID_10.x, BG_ID_10.y),
#     # CT_ID_20 = coalesce(CT_ID_10.x, CT_ID_10.y),
#     # BG_ID_20 = coalesce(BG_ID_20.x, BG_ID_20.y),
#     CT_ID_10 = coalesce(CT_ID_10.x, CT_ID_10.y)
#   )
# # # rename
...
# 
streets_prefinal <- streets_prefinal[,names(roads)]

# Save road files
# export also as csv
write.csv(streets_prefinal,"roads_fin_clust_2024.csv", row.names = F)
...
streets_prefinal_shp <- streets_prefinal_shp[,c(names(roads),"geometry")]
names(streets_prefinal_shp) <- names(roads_shp)
# Convert problematic columns to character
streets_prefinal_shp <- streets_prefinal_shp %>%
  mutate(
    BG_ID_1 = as.character(BG_ID_1),
    CT_ID_1 = as.character(CT_ID_1),
    BG_ID_2 = as.character(BG_ID_2),
    CT_ID_2 = as.character(CT_ID_2)
  )
streets_prefinal_shp$TLID <- as.character(streets_prefinal_shp$TLID)
# export the new shapefile
## run this code in console; otherwise will never finish
st_write(streets_prefinal_shp,"roads_fin_clust_2024.shp")
