...
PAD_unit <- read.csv("~/Desktop/BARI/GI-2023/new datasets/Property Assesments/fy2023-property-assessment-data.csv")
#PAD_unit <- propassraw
dim(PAD_unit) # 177091, 80 in July, 2022
#172266,81 in 2018, 174074 in 2019, 175004 80 in 2020, 177091 70 in 2021, 180627  60 in 2023
...
MtchedTLID <- read.csv("~/Desktop/BARI/GI-2023/Outputs_New/parcels_fullupdatedTLID_August23_YET.csv")
sum(is.na(MtchedTLID$TLID)) #66 in 2019 #65 in 2020 #48 in 2021 #45 in July, 2022, # 44 in March, 2023, #6423 in August 2023
...
prpty_to_prcl <- read.csv("~/Desktop/BARI/GI-2023/Outputs_New/PropertiesToParcels-August23_YET.csv")
...
MtchedTLID$Land_Parcel_ID <- prpty_to_prcl$Land_Parcel_ID[match(MtchedTLID$MAP_PAR_ID,prpty_to_prcl$GIS_ID)] 
# 98857 Unique in March, 2023
#97954 unique before
...
nrow(MtchedTLID)
# [1] 99957 in March, 2023 # [1] 98930 in the last version
...
# [1] 98822 in March 2023 # 98644 in Aug 23
# [1]  #98903 in 2020, 98902 in 2021
length(unique(MtchedTLID$Land_Parcel_ID))
# 98060 in March 2023 # NA in Aug 23
# [1] 98098 # 98051 in 2019 #97954 in 2020 # 97912 in 2021

#too much NA here?????
# NA increased, due to change from 10 to 20. Comment by Toshi Feb 2023. 
sum(is.na(MtchedTLID$Land_Parcel_ID)) #99 in 2023 #157 in 2019 #277 in 2020 # 337 in 2021, 13 missing in 2022
sum(is.na(MtchedTLID$BG_ID_20)) #6 in 2023 #10 in 2020 #10 in 2021 #13 missing in 2022
sum(is.na(MtchedTLID$CT_ID_20)) #6 in  2023 # 8 in 2021 # 8 missing in 2022
sum(is.na(MtchedTLID$Blk_ID_20)) #6 in 2023 #8 in 2021 #8 missing in 2022
sum(is.na(MtchedTLID$TLID)) # 44 in March 2023 # 66 in 2019 #65 in 2020 #48 in 2021 # 43 missing in 2022
length(unique(PAD_unit$GIS_ID)) # 98639 in Aug 23 #98873 in Feb 2023# 98907 in 2019 #99777 in 2020 #98872 in 2021 #98943 in 2022 
length(unique(prpty_to_prcl$GIS_ID)) #98873 in 2023 #98907 in 2019 #99777 in 2020 #98873 in 2021 #98943 in 2022

MtchedTLID <- MtchedTLID %>%
  dplyr::select("MAP_PAR_ID", "TLID", "Land_Parcel_ID", "CT_ID_20", "BG_ID_20", "Blk_ID_20","X","Y")
#Columns "Blk_ID_10", `BG_ID_10` and `CT_ID_10` don't exist in August, 2023
```
...
new4 <- rbind(as.data.frame(new1),as.data.frame(new3))
...
new5 <- new %>%
  ...
  dplyr:: mutate(
		...
		property_N= n(),
		property_N = max(property_N, na.rm=TRUE)) %>%
    dplyr::select("Land_Parcel_ID","AV_TOTAL","AV_LAND","AV_BLDG" ,"LAND_SF","GROSS_AREA","LIVING_AREA","OWN_OCC","NUM_FLOORS","property_N", "unit_N","unit_N_orig" )

new_parcels <- new5 # 177091 in Feb, 2023 #109,843 in 2020, #111,153  in 2021
summary(new_parcels)
# View(new_parcels)
...
new_parcels <- new_parcels[!duplicated(new_parcels),] #98150 in Feb 2023
...
end1 <- Sys.time()
end1-start1 # 33 minutes #Time difference of 9.953541 mins in Aug 23
new_parcels_sp_geo <- new_parcels_sp  #98150 in Feb 2023 #98445 in 2020, #98136 in 2021
...
sum(is.na(new_parcels_sp_geo$lib))#499 in aug23 #475 missing in Feb 2023, 
...
sum(is.na(new_parcels_sp_geo$TLID)) #97906 in aug 23 #104 in 2023  #171 missing in 2019 #534 missing in 2020 #81 missing in 2021
sum(is.na(new_parcels_sp_geo$Blk_ID_20)) #65 in 2023 #137 missing in 2019 #500 in 2020 #47 missing in 2021
sum(is.na(new_parcels_sp_geo$BG_ID_20))  #65 in 2023 #139 missing in 2019 #502 in 2020 #49 missing in 2021
sum(is.na(new_parcels_sp_geo$CT_ID_20)) #65 in 2023 #137 missing in 2019 #500 in 2020  #47 missing in 2021
...
blocks <- st_read("BlocksBostonBARI.shp")
blocks_2020 <- st_read("Census2020_BlockGroups.shp")
...
length(unique(new_parcels_sp_geo$Blk_ID_20)) #5156 in 2023
#5479 in 2019 #5479 in 2020 #5480 in 2021 (some pb here initialy)
length(unique(new_parcels_sp_geo$NSA_Name)) #69 in 2020 ?? new 1
length(unique(new_parcels_sp_geo$BRA_PD))  #17 in 2020 ?? new 1
...
write.csv(new_parcels_sp_geo, "Parcel_final_20230405_TY.csv", row.names = FALSE, na='') #98150 in Feb 2023, #98179 in 2019 #98445 in 2020, # 98136, 37 in 2021
...
parcels_shp <- read_sf("BARIParcels2022.shp")

parcels_shp$GIS_ID <- as.numeric(parcels_shp$GIS_ID)
sum(parcels_shp$GIS_ID %in% MtchedTLID$GIS_ID) #69183 in 2019 - this was because of charact instead of numeric #98930 in 2020 #98695 in 2021 #98857 in Feb 2023
length(unique(parcels_shp$GIS_ID)) #98905 in 2019 #98903 in 2020 #98822 in 2021, #98879 in 2022 #98822 in March 2023

parcels_shp <- parcels_shp %>%
	dplyr:: select(GIS_ID)
parcels_shp$GIS_ID <- as.numeric(parcels_shp$GIS_ID)

length(unique(MtchedTLID$GIS_ID)) #98903 in 2020 #98902 in 2021 #98822  in March 2023
length(unique(MtchedTLID$Land_Parcel_ID)) #97954 in 2020 # 98231 in 2018 #97912 in 2021 #98060 in March, 2023
...
sum(is.na(parcels_shp$Land_Parcel_ID))/length(parcels_shp$Land_Parcel_ID) #0.001586 not matched in 2019 #0.00279996 in 2020 #0.0025997 in 2021 #0.001001447 in March, 2023
# some checks
length(unique(parcels_shp$GIS_ID))  #98822 in 2021 #98903 in 2022 #98822 in 2023
length(unique(parcels_shp$Land_Parcel_ID)) #97911 in 2021 #97954 in 2022 #98060 in 2023
...
#View(table(new_parcels_sp_geo$Blk_ID_10))
length(unique(PAD_unit$Blk_ID_20)) #5480 in 2018 #5479 in 2019 #5479 in 2020 #5478 in 2021 #5156 in March, 2023
...
# save file
write.csv(PAD_unit, "properties_geo_20230331.csv", row.names = FALSE) # 177091, 25 in 2023
