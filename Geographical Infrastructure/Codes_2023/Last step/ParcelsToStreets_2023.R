...
length(unique(parcels$TLID)) #13836 (parcels from script 03) #13890 new #13885 in 2019 #13827 in 2020 #13833 in 2020 with changes #2021: 13817 #13881 in 2023 Dec, 13866 in late2023
...
# 13210 in 2021 #13273 in 2023 Dec 13259 in late2023
...
summary(streets_prefinal) # 11056 streets table(is.na(streets_prefinal$unit_N)) because 11056 street segments do not have parcels #11002 new #11007 in 2019 #11065 in 2020 #11059 in 2020 #11075 in 2021 #11619 in Dec 2023 #11633
...
streets_tracts_10 <- aggregate(CT_ID_10~TLID, data=parcels, FUN=my_mode)
streets_bgs_10 <- aggregate(BG_ID_10~TLID, data=parcels, FUN=my_mode)
streets_tracts_20 <-aggregate(CT_ID_20~TLID, data=parcels, FUN=my_mode)
streets_bgs_20 <-aggregate(BG_ID_20~TLID, data=parcels, FUN=my_mode)

streets_census_10 <-merge(streets_tracts,streets_bgs,by='TLID',all.x=TRUE) #13832 streets_census #13886 new #13881 in 2019 #13823 in 2020 #13828 in fixed one # 13813 in 2021 #13863 in Sep 2023
streets_census_20 <-merge(streets_tracts_20,streets_bgs_20,by='TLID',all.x=TRUE) #13832 streets_census #13886 new #13881 in 2019 #13823 in 2020 #13828 in fixed one # 13813 in 2021 #13863 in Sep 2023

#list_of_dfs <- list(streets_tracts_10, streets_bgs_10, streets_tracts_20, streets_bgs_20)
#streets_census_all <- Reduce(function(x, y) merge(x, y, by = "TLID", all = TRUE), list_of_dfs) #13864 in Dec 23
...
streets_census <- streets_census_all
...
prop <- read.csv("~/Desktop/BARI/GI-2023/drive/Properties/Properties.2023.csv")

# check in the property file the TLID and CT_ID of the missing value from streets_census
View(prop[which(prop$BG_ID_20 == 250250406001 & prop$TLID == 85736902),]) #two properties have these values
...
streets_census_merged <- merge(streets_census,streets_census_20,by='TLID',all.x=TRUE)
...
parcel_shp <- st_read("~/Desktop/BARI/GI-2023/drive/Sanity Checks/LandParcels_2023/LandParcels_2023.shp")
landparcels_sub
# 
# # join the info from the csv in the shp
parcel_shp$Land_Parcel_ID <- parcel_shp$Ln_P_ID
parcels <- left_join(parcel_shp, parcels, by = "Land_Parcel_ID")
...
streets_prefinal <- left_join(roads[c("geometry", "TLID")], streets_prefinal, by="TLID")
streets_prefinal <- st_as_sf(streets_prefinal, crs = 26986)
...
roads_shp_noCT <- streets_prefinal[is.na(streets_prefinal$BG_ID_20),] #24
...
dist_filled <- dist %>%
  mutate(
    BG_ID_20 = coalesce(BG_ID_10.x, BG_ID_10.y),
...
# Save road files
# export also as csv
write.csv(streets_prefinal,"roads_fin_clust_Dec2023.csv", row.names = F)
...
st_write(streets_prefinal,"roads_fin_clust_Dec2023.shp")
