...
parcel_final_2021_csv<-read.csv("Parcel_final_2022_12092022.csv")
...
#parcel_final_2021_shp <- st_read("BARIParcels2022.shp")
#if running the code sequentially from previous script...
parcel_final_2021_shp <- parcels_shp

# join the info from the csv in the shp
#parcel_final_2021_shp$Land_Parcel_ID <- parcel_final_2021_shp$Ln_P_ID
parcels <- left_join(parcel_final_2021_shp, parcel_final_2021_csv, by = "Land_Parcel_ID")
...
streets_final <- st_read("roads_fin_clust_2022_12132022.shp")
...
parcels_LU_count <- dplyr::summarise(group_by(parcels,TLID,LU),count =n())
length(unique(parcels_LU_count$TLID)) #13836 because only 13836 street segments have associated min one parcel #13890 new #13885 in 2019 #13827 in 2020 # 13817 in 2021
...
a <- parcels_LU_count_4 %>%
 dplyr::select("TLID","clusternew")
 
# join the clusters with the streets
streets_clust <- left_join(streets_final, a, by = "TLID", all.x=TRUE)

sum(is.na(streets_clust$cluster)) #11002 #11007 in 2019 #11059 in 2020 # 11075 in 2021
sum(is.na(streets_clust$clusternew))
#11056 street segments are not associated to parcels, so they do not have LU
sum(!is.na(streets_clust$clusternew)) #13889 #13884 in 2019 #13832 in 2020 # 13816 in 2021
sum(!is.na(streets_clust$cluster))
#13835 street segments associated to parcels #13826 in 2020 #13816 in 2021

# add 0's instead of NA's for the streets without clusters
streets_clust[is.na(streets_clust)] <- 0
sum(streets_clust$clusternew == 0) # 11056 #11002 new #11007 in 2019 #11059 in 2020 # 11075 in 2021
...
write.csv(streets_clust2,"roads_fin_clust_2022_12132022.csv")
#st_write(streets_clust,"../Outputs/Streets/roads_fin_clust_2021_08302021.shp")


# Calculate percentages for the documentation 
# first add the parcel data
parcels_2021 <- read.csv("Parcel_final_2022_12092022.csv") #98445
