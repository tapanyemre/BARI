...
PAD_unit <- read.csv("~/Desktop/BARI/PA-2023/outputs_Feb24/PAD.Record.wUnit.2022.csv")
#PAD_unit <- propassraw
dim(PAD_unit) # 180627 67 in 2023 
...
MtchedTLID <- read.csv("~/Desktop/BARI/GI-2023/Outputs_Feb24/parcels_updatedTLID_2023.csv")
sum(is.na(MtchedTLID$TLID)) #66 in 2019 #65 in 2020 #48 in 2021 #45 in July, 2022, # 44 in March, 2023, #96 in Sep 2023
...
prpty_to_prcl <- read.csv("GI-2023/drive/PropertiesToParcels_10032023_YET.csv")
# 206958 in 2018 # 198269 in 2019 #176925 in July 2022, #176925 in March 2023, #180448 in August 2023

# special parcels from 2019
prcl <- read.csv("~/Desktop/BARI/GI-2023/new datasets/all_specialparcels_10282019.csv") # 98903 in 2018 #98930 in October, 2019

#grab the raw data and merge by PID to PAD_unit
#new_unit <- read.csv("~/Desktop/BARI/PA-2023/outputs/PAD.Record.wUnit08242023.csv")

#new_unit <- new_unit %>% dplyr::select(PID, unit_N, unit_N_orig, New_unit)

#PAD_unit <- merge(PAD_unit, new_unit, by = "PID", all.x = TRUE)
...
#"OWN_OCC","OWNER",
#"MAIL_ADDRESSEE",
#"MAIL_ADDRESS","MAIL_ZIPCODE",             #"MAIL.CS",
#"YR_BUILT","YR_REMOD",
#"NUM_FLOORS",
...
# 97894 in Sep 23, 98857 Unique in March, 2023
#97954 unique before
...
nrow(MtchedTLID)
#98639 in Sep 23  98964 in Aug 23 [1] 99957 in March, 2023 # [1] 98930 in the last version
#MtchedTLID <- MtchedTLID %>% mutate(GIS_ID = MAP_PAR_ID) #here are problems sometimes because of plyr, remove it
length(unique(MtchedTLID$MAP_PAR_ID))
# [1] 98822 in March 2023 # 98645 in Aug 23 #98639 in Sep 23
# [1]  #98903 in 2020, 98902 in 2021
length(unique(MtchedTLID$Land_Parcel_ID))
# 98060 in March 2023 # 97907 in Aug 23 # 97894 in Sep 23
# [1] 98098 # 98051 in 2019 #97954 in 2020 # 97912 in 2021

#too much NA here?????
# NA increased, due to change from 10 to 20. Comment by Toshi Feb 2023. 
sum(is.na(MtchedTLID$Land_Parcel_ID)) #0 in Sep 23, 99 in 2023 #157 in 2019 #277 in 2020 # 337 in 2021, 13 missing in 2022
sum(is.na(MtchedTLID$BG_ID_20)) #3 in Sep 23, 6 in 2023 #10 in 2020 #10 in 2021 #13 missing in 2022
sum(is.na(MtchedTLID$CT_ID_20)) #3 in Sep 23, 6 in  2023 # 8 in 2021 # 8 missing in 2022
sum(is.na(MtchedTLID$Blk_ID_20)) #3 in Sep 23, 6 in 2023 #8 in 2021 #8 missing in 2022
sum(is.na(MtchedTLID$TLID)) #96 in Sep 23, 44 in March 2023 # 66 in 2019 #65 in 2020 #48 in 2021 # 43 missing in 2022
length(unique(PAD_unit$GIS_ID)) # 98639 in Aug 23 #98873 in Feb 2023# 98907 in 2019 #99777 in 2020 #98872 in 2021 #98943 in 2022 
length(unique(prpty_to_prcl$GIS_ID)) #98639 in 2023 #98907 in 2019 #99777 in 2020 #98873 in 2021 #98943 in 2022
...
new <- PAD_unit #%>%
#dplyr::select(-GROSS_AREA) %>% # remove imputed values
...
#new <- new %>% mutate(property_N = n())

#new$R_BLDG_STYL[new$R_BLDG_STYL==""] <- NA  #### does not exist in 2021

new$owner_address <- paste(new$OWNER.MAIL.ADDRESS,new$ZIP_CODE)
...
new_parcels <- new_parcels[!duplicated(new_parcels),] #97894 in Sep 23,  #98150 in Feb 2023
...
end1 <- Sys.time()
end1-start1 # 33 minutes #Time difference of 11.91908 mins in Sep 23
...
sum(is.na(new_parcels_sp_geo$TLID)) #93 in sep 23 #104 in 2023  #171 missing in 2019 #534 missing in 2020 #81 missing in 2021
sum(is.na(new_parcels_sp_geo$Blk_ID_20)) #2 in sep 23 65 in 2023 #137 missing in 2019 #500 in 2020 #47 missing in 2021
sum(is.na(new_parcels_sp_geo$BG_ID_20))  #2 in sep 23 65 in 2023 #139 missing in 2019 #502 in 2020 #49 missing in 2021
sum(is.na(new_parcels_sp_geo$CT_ID_20)) #2 in sep 23 65 in 2023 #137 missing in 2019 #500 in 2020  #47 missing in 2021
...
blocks <- st_read("~/Desktop/BARI/GI-2023/new datasets/Blocks_Boston_2010_BARI/Blocks_Boston BARI.shp")
blocks_2020 <- st_read("~/Desktop/BARI/GI-2023/new datasets/Block_Groups_Boston_2020/Block_Groups_Boston_2020.shp")
...
length(unique(new_parcels_sp_geo$Blk_ID_20)) #5155 in Sep 23, 5156 in 2023
#5479 in 2019 #5479 in 2020 #5480 in 2021 (some pb here initialy)
length(unique(new_parcels_sp_geo$NSA_Name)) #69 in 2020 #69 in Sep 23
length(unique(new_parcels_sp_geo$BRA_PD))  #17 in 2020 #17 in Sep 23
...
write.csv(new_parcels_sp_geo, "~/Desktop/BARI/GI-2023/outputs/Parcel_final_09302023_YET.csv", row.names = FALSE, na='') #97894in Sep 23, 98150 in Feb 2023, #98179 in 2019 #98445 in 2020, # 98136, 37 in 2021
...
parcels_shp <- read_sf("~/Desktop/BARI/GI-2023/new datasets/Parcel Datasets/Parcels_2023/parcels_23.shp")

parcels_shp$GIS_ID <- as.numeric(parcels_shp$MAP_PAR_ID)
sum(parcels_shp$GIS_ID %in% MtchedTLID$GIS_ID) #69183 in 2019 - this was because of charact instead of numeric #98930 in 2020 #98695 in 2021 #98857 in Feb 2023 #98644 in Sep 23 
length(unique(parcels_shp$GIS_ID)) #98905 in 2019 #98903 in 2020 #98822 in 2021, #98879 in 2022 #98822 in March 2023 #98644 in Sep 23

parcels_shp <- parcels_shp %>%
  dplyr:: select(GIS_ID)
parcels_shp$GIS_ID <- as.numeric(parcels_shp$GIS_ID)

length(unique(MtchedTLID$GIS_ID)) #98903 in 2020 #98902 in 2021 #98822  in March 2023 #98639 in Sep 23
length(unique(MtchedTLID$Land_Parcel_ID)) #97954 in 2020 # 98231 in 2018 #97912 in 2021 #98060 in March, 2023 #97894 in Sep 23
...
sum(is.na(parcels_shp$Land_Parcel_ID))/length(parcels_shp$Land_Parcel_ID) #0.001586 not matched in 2019 #0.00279996 in 2020 #0.0025997 in 2021 #0.001001447 in March, 2023 #0.003233499 in Sep 23
# some checks
length(unique(parcels_shp$GIS_ID))  #98822 in 2021 #98903 in 2022 #98822 in 2023 #98644 in Sep 23
length(unique(parcels_shp$Land_Parcel_ID)) #97911 in 2021 #97954 in 2022 #98060 in 2023 #97895 in Sep 23
...
#View(table(new_parcels_sp_geo$Blk_ID_10))
length(unique(PAD_unit$Blk_ID_20)) #5480 in 2018 #5479 in 2019 #5479 in 2020 #5478 in 2021 #5156 in March, 2023 #5155 in Sep 23
...
# save file
write.csv(PAD_unit, "~/Desktop/BARI/GI-2023/outputs/properties_geo_09302023_YET.csv", row.names = FALSE) #180627, 28 in Sep 23 # 177091, 25 in 2023
#174074 in 2019 #175004 in 2020 # 177091 in 2021 # 172266, 26 in 2022
# where one column difference come from? It seems correct, 2022 is also 25 field
# we add na='' when writing the csv so it can be used in ArcGis (for numeric fields) 
...
# Unusable Toshi comment on Feb 21, 2023
