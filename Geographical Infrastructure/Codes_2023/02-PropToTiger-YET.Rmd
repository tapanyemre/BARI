...
propassraw <- read.csv("~/Desktop/BARI/PA-2023/outputs_new/PAD.Record.wUnit08162023.csv")

# read street data #we are using the csv created for the GI2019 release
??? #tiger22 <- read.csv("Streets_2019_Prefinal_190731.csv")
tiger22 <- read_csv("~/Desktop/BARI/GI-2023/new datasets/Street Data/roads_fin_clust_2022_12132022.csv")

# read shp street data and connect geometry with csv (((UPDATE YEARS HERE< TAKE them from drive)))
tiger21_shp <- st_read("~/Desktop/BARI/GI-2023/new datasets/Street Data/Roads_2021.shp")
tiger22$geometry <- tiger21_shp$geometry[match(tiger22$TLID, tiger21_shp$TLID)]
```
...
propassraw$PID <- as.integer(propassraw$PID)                                             
propass23 <- propassraw[!duplicated(propassraw$GIS_ID),] #36346 in 2023 #98873, as of 2022
 #98873, as of 2022 
#  # 98907 in 2019 #99777 in 2020, #98943 in 2021

# Finding the centroid and merging it back unto the parcel shapefile
#P21shp_centroid <- st_centroid(parcels21_shp %>% st_transform(26986)) 
#st_centroid assumes attributes are constant over geometries of x
P23shp_centroid <- st_centroid(parcels23_shp %>%
                                 st_transform(26986))
  
# Merging the property data with the parcels shapefile and 
# converting them to spatial objects (needed to be re-projected)
P23shp_centroid$MAP_PAR_ID <- as.numeric(as.character(P23shp_centroid$MAP_PAR_ID)) # message: NAs introduced by coercion
#????? no column like GIS_ID - create a new column and rename it as GIS_ID -- changed it to MAP_PAR_ID

propass23$GIS_ID <- as.numeric(propass23$GIS_ID)
P23sf <- left_join(P23shp_centroid, propass23, by=c("MAP_PAR_ID"="GIS_ID"))
a <- left_join(propass23, P23shp_centroid, by=c("GIS_ID"="MAP_PAR_ID"))
...
P23sf$FULLNAME <- str_c(P23sf$ST_NAME, P23sf$ST_NAME_SUF2, sep = " ")
...
#3678 in August, 2023
#4060 in March,2023
#4087 in 2020 #4056 in 2021 #4061 in 2022 #4346 in 2023
```
...
tiger22 <- st_transform(tiger22, 26986)
...
start1 <- Sys.time()
pb <- progress_estimated(nrow(P23sf),0)

P23sf$TLID <- NA

for(i in 1:nrow(P23sf)){
  # i = 12
  matching_streets <- tiger22 %>%
    filter(FULLNAM %in% P23sf$FULLNAM[i])
  if(nrow(matching_streets)>0){
    P23sf$TLID[i] <- as.character(matching_streets$TLID[which.min(st_distance(P23sf[i,"geometry"], matching_streets))])
    }
   pb$tick()$print() # update progress bar
}

end1 <- Sys.time()
end1-start1 #Time difference of 8.631304 mins


sum(is.na(P23sf$TLID))/nrow(P23sf) #0.7699264 in Aug 23 #  0.01430806 in Feb 19th,2023  
# 0.01690084 in 2020 #0.0173456 in 2021 # 0.01544526 in 2022
table(P23sf$FULLNAM[which(is.na(P23sf$TLID))]) %>% sort()
```
...
length(unique(P23sf$FULLNAME[which(is.na(P23sf$TLID) & grepl("ISLAND",P23sf$FULLNAME)==F)])) 
#988 in Aug 23
# 206 in Feb 2023
...
blocks_10 <- st_transform(blocks_10,26986)
...
P23sf_geo <- st_join(P23sf,blocks_10,join=st_within,left=T)
P23sf_geo <- st_join(P23sf,blocks,join=st_within,left=T)
P23sf_geo <- st_join(P23sf_geo,bgs,join=st_within,left=T)

sum(is.na(P23sf_geo$Blk_ID_20))/nrow(P23sf_geo)  #0.000191989 in Aug 23 #0.0002225437 in March 2023#0.0002425958 in 2020
sum(is.na(P23sf_geo$BG_ID_20))/nrow(P23sf_geo) #0.000191989 in Aug 23 #0.0002225437 in March 2023
sum(is.na(P23sf_geo$CT_ID_20))/nrow(P23sf_geo) #0.000191989 in Aug 23 #0.0002225437 in March 2023 #0.00024259 in 2020
...
write_csv(as.data.frame(st_set_geometry(P23sf_geo,NULL)),"parcels_updatedTLID_August23_YET.csv")
```
...
```{r}
P23sf_geo <- read.csv("parcels_updatedTLID_20230803_YET.csv")
```
...
P22 <- st_as_sf(P23sf, coords = c("X", "Y"), crs = 26986)
sum(is.na(P22$TLID)) #28885 in Aug 23, #1149 in March 2023. 
...
# %>%
#  head(n = 20)
# TLID_NA <- st_join(TLID_NA, P22[,"ZIPCODE"],join=st_within,left=T)
# TLID_NA <- P22[(is.na(P22$TLID)),]
...
TLID_NA <- P22shp_centroid%>%
  filter(MAP_PAR_ID %in% P22$MAP_PAR_ID[is.na(P22$TLID)]) # this will have 9 columns
...
# Time difference of 1.481762 hours in 2023
# 1418 ?
...
names = c(colnames(P22shp_centroid))
...
# save as csv 
write.csv(TLID_NA_2, "TLIDs_August23_YET.csv", row.names = FALSE)


#### Join data: parcels with TLID, parcels run in the buffer loop and resulted with or without TLIDs ####

dt <- left_join(as.data.frame(P22), as.data.frame(TLID_new[,c("TLID", "MAP_PAR_ID")]), by = "MAP_PAR_ID")
...
# save as csv
write.csv(as.data.frame(st_set_geometry(dt,NULL)),"parcels_fullupdatedTLID_August23_YET.csv", row.names = FALSE)
```
